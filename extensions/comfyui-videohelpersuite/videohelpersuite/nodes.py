...
def to_pingpong_trimmed(inp, target_frames):
    """Pingpong the input and trim to target number of frames"""
    if not hasattr(inp, "__getitem__"):
        inp = list(inp)
    ...
    if input_len >= target_frames:
        # If input is already longer or equal, just yield target frames
        for i in range(target_frames):
            yield inp[i]
        return

    # Calculate frames per pingpong cycle
    if input_len == 1:
    ...
    # For multiple frames: full cycle is len + (len-2) = 2*len - 2
    cycle_length = 2 * input_len - 2
    frames_yielded = 0

    while frames_yielded < target_frames:
        # Forward pass
        for i in range(input_len):
            if frames_yielded >= target_frames:
                return
            yield inp[i]
            frames_yielded += 1

        # Backward pass (excluding first and last frame)
        for i in range(input_len - 2, 0, -1):
            if frames_yielded >= target_frames:
                return
            yield inp[i]
            frames_yielded += 1
...
class VideoCombine:
    ...
    def combine_video(
        ...
        **kwargs
        ...
        # Handle pingpong_to_audio: if video is shorter than audio, pingpong and trim to audio length
        # Get from kwargs (format-specific widget)
        target_frames = None
        pingpong_to_audio_enabled = kwargs.get("pingpong_to_audio", False)
        if pingpong_to_audio_enabled and audio is not None:
            try:
                # Check if audio exists
                a_waveform = audio.get('waveform')
                if a_waveform is not None:
                    # Calculate audio duration in seconds
                    audio_duration = a_waveform.size(2) / audio['sample_rate']
                    # Calculate required frames to match audio duration
                    target_frames = int(audio_duration * frame_rate)

                    # Only apply if video is shorter than audio
                    if num_frames < target_frames:
                        # Convert images to list if needed (for pingpong iteration)
                        if not hasattr(images, "__getitem__"):
                            images = list(images)
                        # Apply pingpong and trim to audio length
                        images = to_pingpong_trimmed(images, target_frames)
                        # Update num_frames and progress bar
                        num_frames = target_frames
                        pbar.total = num_frames
            except Exception:
                # If there's any issue with audio, just continue without pingpong_to_audio
                pass
        ...
        if format_type == "image":
            ...
            # Don't apply regular pingpong if pingpong_to_audio was already applied
            if pingpong and not (pingpong_to_audio_enabled and target_frames is not None):
